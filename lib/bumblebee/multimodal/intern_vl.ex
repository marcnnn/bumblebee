defmodule Bumblebee.Multimodal.InternVl do
  ...
  options =
    [
      vision_spec: [
        default: nil,
        doc:
          "the specification of the vision model. See `Bumblebee.Vision.InternVit` for details"
      ],
      text_spec: [
        default: nil,
        doc: "the specification of the text model. See `Bumblebee.Text.InternLm2` for details"
      ]
    ]

  @moduledoc """
  The InternVL model for visual language understanding.

  InternVL composes an InternViT vision encoder with an InternLM2 text
  decoder via an MLP projection layer.

  ## Architectures

    * `:for_conditional_generation` - InternVL model with a language
      modeling head
  ...
  def model(%__MODULE__{architecture: :for_conditional_generation} = spec) do
    ...
    # MLP projection: two dense layers with GELU activation
    # Maps from vision hidden size to text hidden size
  ...
  defimpl Bumblebee.HuggingFace.Transformers.Config do
    def load(spec, data) do
      ...
      {text_data, data} = Map.pop(data, "llm_config", %{})

      vision_spec =
        Bumblebee.Vision.InternVit
        |> Bumblebee.configure()
        |> Bumblebee.HuggingFace.Transformers.Config.load(vision_data)

      text_spec =
        Bumblebee.Text.InternLm2
