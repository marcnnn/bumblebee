defmodule Bumblebee.Text.GptJ do
  ...
  options =
    [
      ...
      intermediate_size: [
        default: 16384,
        doc: "the dimensionality of the intermediate layer in the transformer feed-forward network (FFN) in the decoder"
      ],
      activation: [
  ...
  ## Architectures
    ...
    * `:base` - plain GPT-J without any head on top

    * `:for_causal_language_modeling` - GPT-J with a language modeling
      head. The head returns logits for each token in the original
      sequence

    * `:for_sequence_classification` - GPT-J with a sequence
      classification head. The head returns logits corresponding to
      possible classes

    * `:for_question_answering` - GPT-J with a span classification head.
