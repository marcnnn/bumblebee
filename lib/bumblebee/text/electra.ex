defmodule Bumblebee.Text.Electra do
  ...
  options =
    [
      ...
      embedding_size: [
        default: 128,
        doc: "the dimensionality of the embedding projection layer"
      ],
      num_blocks: [
        default: 12,
  ...
  @moduledoc """
  ELECTRA model family.

  ## Architectures

    * `:base` - plain ELECTRA without any head on top

    * `:for_masked_language_modeling` - ELECTRA with a language modeling
      head. The head returns logits for each token in the original
      sequence

    * `:for_sequence_classification` - ELECTRA with a sequence
      classification head. The head returns logits corresponding to
      possible classes

    * `:for_token_classification` - ELECTRA with a token classification
      head. The head returns logits for each token in the original
      sequence

    * `:for_question_answering` - ELECTRA with a span classification head.
      The head returns logits for the span start and end positions

    * `:for_multiple_choice` - ELECTRA with a multiple choice prediction
    ...
    * `:for_pre_training` - ELECTRA with a discriminator head that
      predicts whether each token in the sequence is original or replaced
  ...
  def model(%__MODULE__{architecture: :for_sequence_classification} = spec) do
    ...
    logits =
      ...
      |> Axon.dropout(
        rate: classifier_dropout_rate(spec),
        name: "sequence_classification_head.dropout2"
      )
  ...
  def model(%__MODULE__{architecture: :for_pre_training} = spec) do
    ...
    logits =
      ...
      |> Axon.dense(spec.hidden_size,
        kernel_initializer: kernel_initializer(spec),
        name: "discriminator_head.dense"
      )
      |> Axon.activation(:gelu)
      |> Axon.dense(1,
        kernel_initializer: kernel_initializer(spec),
        name: "discriminator_head.output"
      )
  ...
  defp embedder(input_ids, position_ids, token_type_ids, spec, opts) do
    ...
    if spec.embedding_size != spec.hidden_size do
      Axon.dense(embeddings, spec.hidden_size,
        kernel_initializer: kernel_initializer(spec),
  ...
  defimpl Bumblebee.HuggingFace.Transformers.Model do
    def params_mapping(_spec) do
      %{
        "embedder.token_embedding" => "electra.embeddings.word_embeddings",
        "embedder.position_embedding" => "electra.embeddings.position_embeddings",
        "embedder.token_type_embedding" => "electra.embeddings.token_type_embeddings",
        "embedder.norm" => "electra.embeddings.LayerNorm",
        "embedder.projection" => "electra.embeddings_project",
        "encoder.blocks.{n}.self_attention.query" =>
          "electra.encoder.layer.{n}.attention.self.query",
        "encoder.blocks.{n}.self_attention.key" =>
          "electra.encoder.layer.{n}.attention.self.key",
        "encoder.blocks.{n}.self_attention.value" =>
          "electra.encoder.layer.{n}.attention.self.value",
        "encoder.blocks.{n}.self_attention.output" =>
          "electra.encoder.layer.{n}.attention.output.dense",
        "encoder.blocks.{n}.self_attention_norm" =>
          "electra.encoder.layer.{n}.attention.output.LayerNorm",
        "encoder.blocks.{n}.ffn.intermediate" => "electra.encoder.layer.{n}.intermediate.dense",
        "encoder.blocks.{n}.ffn.output" => "electra.encoder.layer.{n}.output.dense",
        "encoder.blocks.{n}.output_norm" => "electra.encoder.layer.{n}.output.LayerNorm",
        "language_modeling_head.dense" => "generator_predictions.dense",
        "language_modeling_head.norm" => "generator_predictions.LayerNorm",
        "language_modeling_head.output" => "generator_lm_head",
        "language_modeling_head.bias" => "generator_lm_head",
        "discriminator_head.dense" => "discriminator_predictions.dense",
        "discriminator_head.output" => "discriminator_predictions.dense_prediction",
