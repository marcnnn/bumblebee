defmodule Bumblebee.Text.StarCoder2 do
  ...
  options =
    [
      ...
      layer_norm_epsilon: [
        default: 1.0e-5,
        doc: "the epsilon used by normalization layers"
      ],
      initializer_scale: [
        default: 0.02,
        doc:
  ...
  @moduledoc """
  StarCoder2 model family.

  ## Architectures

    * `:base` - plain StarCoder2 without any head on top

    * `:for_causal_language_modeling` - StarCoder2 with a language modeling
      head. The head returns logits for each token in the original
      sequence

    * `:for_sequence_classification` - StarCoder2 with a sequence
      classification head. The head returns logits corresponding to
      possible classes

    * `:for_token_classification` - StarCoder2 with a token classification
      head. The head returns logits for each token in the original
      sequence
  ...
  defp gated_ffn(hidden_state, intermediate_size, output_size, opts) do
    ...
    gate = Axon.dense(hidden_state, intermediate_size, name: join(name, "gate"), use_bias: true)

    hidden_state = Axon.multiply(intermediate, Axon.activation(gate, activation))

    Axon.dense(hidden_state, output_size, name: join(name, "output"), use_bias: true)
  end
    ...
    def load(spec, data) do
      ...
      opts =
        convert!(data,
          ...
          layer_norm_epsilon: {"norm_epsilon", number()}
