defmodule Bumblebee.Text.FunnelTransformer do
  ...
  @moduledoc """
  Funnel Transformer model family.

  ## Architectures

    * `:base` - plain Funnel Transformer without any head on top

    * `:for_masked_language_modeling` - Funnel Transformer with a language modeling
      head. The head returns logits for each token in the original
      sequence

    * `:for_sequence_classification` - Funnel Transformer with a sequence
      classification head. The head returns logits corresponding to
      possible classes

    * `:for_token_classification` - Funnel Transformer with a token classification
      head. The head returns logits for each token in the original
      sequence

    * `:for_question_answering` - Funnel Transformer with a span classification head.
      The head returns logits for the span start and end positions

    * `:for_multiple_choice` - Funnel Transformer with a multiple choice prediction
  ...
  defimpl Bumblebee.HuggingFace.Transformers.Config do
    def load(spec, data) do
      ...
      # Funnel Transformer uses block_sizes to define total layers;
      # we sum them to get num_blocks
      num_blocks =
        case data["block_sizes"] do
          sizes when is_list(sizes) -> Enum.sum(sizes)
          _ -> nil
        end
      ...
      opts =
        if num_blocks do
          Keyword.put(opts, :num_blocks, num_blocks)
        else
          opts
        end
  ...
  defimpl Bumblebee.HuggingFace.Transformers.Model do
    def params_mapping(_spec) do
      %{
        "embedder.token_embedding" => "funnel.embeddings.word_embeddings",
        "embedder.position_embedding" => "funnel.embeddings.position_embeddings",
        "embedder.token_type_embedding" => "funnel.embeddings.token_type_embeddings",
        "embedder.norm" => "funnel.embeddings.LayerNorm",
        "encoder.blocks.{n}.self_attention.query" =>
          "funnel.encoder.blocks.{n}.attention.q_head",
        "encoder.blocks.{n}.self_attention.key" =>
          "funnel.encoder.blocks.{n}.attention.k_head",
        "encoder.blocks.{n}.self_attention.value" =>
          "funnel.encoder.blocks.{n}.attention.v_head",
        "encoder.blocks.{n}.self_attention.output" =>
          "funnel.encoder.blocks.{n}.attention.post_proj",
        "encoder.blocks.{n}.self_attention_norm" =>
          "funnel.encoder.blocks.{n}.attention.layer_norm",
        "encoder.blocks.{n}.ffn.intermediate" =>
          "funnel.encoder.blocks.{n}.ffn.linear_1",
        "encoder.blocks.{n}.ffn.output" =>
          "funnel.encoder.blocks.{n}.ffn.linear_2",
        "encoder.blocks.{n}.output_norm" =>
          "funnel.encoder.blocks.{n}.ffn.layer_norm",
        "pooler.output" => "funnel.pooler.dense",
