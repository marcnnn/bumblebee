defmodule Bumblebee.Text.Roformer do
  ...
  options =
    [
      ...
      rotary_value: [
        default: false,
        doc: "whether to apply rotary position embedding to value states as well"
      ]
    ] ++ Shared.common_options([:num_labels, :id_to_label])

  @moduledoc """
  RoFormer model family.

  ## Architectures

    * `:base` - plain RoFormer without any head on top

    * `:for_masked_language_modeling` - RoFormer with a language modeling
      head. The head returns logits for each token in the original
      sequence

    * `:for_sequence_classification` - RoFormer with a sequence
      classification head. The head returns logits corresponding to
      possible classes

    * `:for_token_classification` - RoFormer with a token classification
      head. The head returns logits for each token in the original
      sequence

    * `:for_question_answering` - RoFormer with a span classification head.
      The head returns logits for the span start and end positions

    * `:for_multiple_choice` - RoFormer with a multiple choice prediction
  ...
  defp embedder(input_ids, token_type_ids, spec, opts) do
    ...
    # RoFormer uses rotary position embeddings instead of absolute position embeddings,
    # so we only add token + token_type embeddings here
    ...
    def load(spec, data) do
      ...
      opts =
        convert!(data,
          ...
          rotary_value: {"rotary_value", optional(boolean())}
  ...
  defimpl Bumblebee.HuggingFace.Transformers.Model do
    def params_mapping(_spec) do
      %{
        "embedder.token_embedding" => "roformer.embeddings.word_embeddings",
        "embedder.token_type_embedding" => "roformer.embeddings.token_type_embeddings",
        "embedder.norm" => "roformer.embeddings.LayerNorm",
        "encoder.blocks.{n}.self_attention.query" =>
          "roformer.encoder.layer.{n}.attention.self.query",
        "encoder.blocks.{n}.self_attention.key" =>
          "roformer.encoder.layer.{n}.attention.self.key",
        "encoder.blocks.{n}.self_attention.value" =>
          "roformer.encoder.layer.{n}.attention.self.value",
        "encoder.blocks.{n}.self_attention.output" =>
          "roformer.encoder.layer.{n}.attention.output.dense",
        "encoder.blocks.{n}.self_attention_norm" =>
          "roformer.encoder.layer.{n}.attention.output.LayerNorm",
        "encoder.blocks.{n}.self_attention.rotary_embedding" =>
          "roformer.encoder.layer.{n}.attention.self.rotary_embedding",
        "encoder.blocks.{n}.ffn.intermediate" =>
          "roformer.encoder.layer.{n}.intermediate.dense",
        "encoder.blocks.{n}.ffn.output" => "roformer.encoder.layer.{n}.output.dense",
        "encoder.blocks.{n}.output_norm" => "roformer.encoder.layer.{n}.output.LayerNorm",
        "pooler.output" => "roformer.pooler.dense",
