defmodule Bumblebee.Text.Longformer do
  ...
  options =
    [
      ...
      attention_window_size: [
        default: 512,
        doc: "the size of the sliding window attention"
      ]
    ] ++ Shared.common_options([:num_labels, :id_to_label])

  @moduledoc """
  Longformer model family.

  ## Architectures

    * `:base` - plain Longformer without any head on top

    * `:for_masked_language_modeling` - Longformer with a language modeling
      head. The head returns logits for each token in the original
      sequence

    * `:for_sequence_classification` - Longformer with a sequence
      classification head. The head returns logits corresponding to
      possible classes

    * `:for_token_classification` - Longformer with a token classification
      head. The head returns logits for each token in the original
      sequence

    * `:for_question_answering` - Longformer with a span classification head.
      The head returns logits for the span start and end positions

    * `:for_multiple_choice` - Longformer with a multiple choice prediction
  ...
  defimpl Bumblebee.HuggingFace.Transformers.Config do
    def load(spec, data) do
      ...
      attention_window_size =
        case data["attention_window"] do
          [size | _] when is_integer(size) -> size
      ...
      opts =
        if attention_window_size do
          Keyword.put(opts, :attention_window_size, attention_window_size)
        else
          opts
        end
  ...
  defimpl Bumblebee.HuggingFace.Transformers.Model do
    def params_mapping(_spec) do
      %{
        "embedder.token_embedding" => "longformer.embeddings.word_embeddings",
        "embedder.position_embedding" => "longformer.embeddings.position_embeddings",
        "embedder.token_type_embedding" => "longformer.embeddings.token_type_embeddings",
        "embedder.norm" => "longformer.embeddings.LayerNorm",
        "encoder.blocks.{n}.self_attention.query" =>
          "longformer.encoder.layer.{n}.attention.self.query",
        "encoder.blocks.{n}.self_attention.key" =>
          "longformer.encoder.layer.{n}.attention.self.key",
        "encoder.blocks.{n}.self_attention.value" =>
          "longformer.encoder.layer.{n}.attention.self.value",
        "encoder.blocks.{n}.self_attention.output" =>
          "longformer.encoder.layer.{n}.attention.output.dense",
        "encoder.blocks.{n}.self_attention_norm" =>
          "longformer.encoder.layer.{n}.attention.output.LayerNorm",
        "encoder.blocks.{n}.ffn.intermediate" =>
          "longformer.encoder.layer.{n}.intermediate.dense",
        "encoder.blocks.{n}.ffn.output" => "longformer.encoder.layer.{n}.output.dense",
        "encoder.blocks.{n}.output_norm" => "longformer.encoder.layer.{n}.output.LayerNorm",
        "pooler.output" => "longformer.pooler.dense",
