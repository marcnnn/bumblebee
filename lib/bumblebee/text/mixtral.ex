defmodule Bumblebee.Text.Mixtral do
  ...
  options =
    [
      ...
      num_experts: [
        default: 8,
        doc: "the number of experts in the Mixture of Experts layer"
      ],
  ...
  ## Architectures
    ...
    * `:base` - plain Mixtral without any head on top

    * `:for_causal_language_modeling` - Mixtral with a language modeling
      head. The head returns logits for each token in the original
      sequence

    * `:for_sequence_classification` - Mixtral with a sequence
  ...
  dense FFN layer (same as Mistral). This allows loading dense or distilled Mixtral
  weights but does not implement the full sparse MoE routing.
  """
  ...
  defimpl Bumblebee.HuggingFace.Transformers.Model do
    def params_mapping(_spec) do
      %{
        ...
        "decoder.blocks.{n}.ffn.gate" => "model.layers.{n}.block_sparse_moe.experts.0.w1",
        "decoder.blocks.{n}.ffn.intermediate" =>
          "model.layers.{n}.block_sparse_moe.experts.0.w3",
        "decoder.blocks.{n}.ffn.output" => "model.layers.{n}.block_sparse_moe.experts.0.w2",
