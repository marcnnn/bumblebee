defmodule Bumblebee.Text.Deberta do
  ...
  @moduledoc """
  DeBERTa model family.

  ## Architectures

    * `:base` - plain DeBERTa without any head on top

    * `:for_masked_language_modeling` - DeBERTa with a language modeling
      head. The head returns logits for each token in the original
      sequence

    * `:for_sequence_classification` - DeBERTa with a sequence
      classification head. The head returns logits corresponding to
      possible classes

    * `:for_token_classification` - DeBERTa with a token classification
      head. The head returns logits for each token in the original
      sequence

    * `:for_question_answering` - DeBERTa with a span classification head.
