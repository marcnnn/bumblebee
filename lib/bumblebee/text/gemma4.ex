defmodule Bumblebee.Text.Gemma4 do
  ...
  options =
    [
      ...
      sliding_window: [
        default: 512,
        doc: "the sliding window size for local attention on sliding attention layers"
      ],
      layer_types: [
        default: nil,
        doc: """
        a list of layer type atoms specifying the attention pattern for each block.
        Valid values are `:sliding_attention` and `:full_attention`.
        When `nil`, defaults to alternating sliding and full attention layers.
        """
      ],
      final_logit_softcapping: [
        default: nil,
        doc: "soft-capping value for final logits"
      ],
      use_attention_bias: [
        default: false,
        doc:
  ...
  defp resolve_layer_types(spec) do
    case spec.layer_types do
      nil ->
        # Default: alternating sliding and full attention
        for i <- 0..(spec.num_blocks - 1) do
          if rem(i, 2) == 0, do: :sliding_attention, else: :full_attention
        end
    ...
    def load(spec, data) do
      ...
      opts =
        convert!(data,
          ...
          layer_types: {"layer_types", optional(layer_types_converter)},
