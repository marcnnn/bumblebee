defmodule Bumblebee.Text.DebertaV2 do
  ...
  options =
    [
      ...
      relative_attention: [
        default: true,
        doc: "whether to use relative position encoding"
      ],
      position_bucket_size: [
        default: 256,
        doc: "the maximum number of relative position buckets"
      ]
    ] ++ Shared.common_options([:num_labels, :id_to_label])

  @moduledoc """
  DeBERTa-v2 model family.

  ## Architectures

    * `:base` - plain DeBERTa-v2 without any head on top

    * `:for_masked_language_modeling` - DeBERTa-v2 with a language modeling
      head. The head returns logits for each token in the original
      sequence

    * `:for_sequence_classification` - DeBERTa-v2 with a sequence
      classification head. The head returns logits corresponding to
      possible classes

    * `:for_token_classification` - DeBERTa-v2 with a token classification
      head. The head returns logits for each token in the original
      sequence

    * `:for_question_answering` - DeBERTa-v2 with a span classification head.
      The head returns logits for the span start and end positions

    * `:for_multiple_choice` - DeBERTa-v2 with a multiple choice prediction
  ...
  defp embedder(input_ids, position_ids, token_type_ids, spec, opts) do
    ...
    if not spec.relative_attention do
      position_embeddings =
    ...
    def load(spec, data) do
      ...
      opts =
        convert!(data,
          ...
          relative_attention: {"relative_attention", boolean()},
          position_bucket_size: {"pos_att_type", optional(number())}
  ...
  defimpl Bumblebee.HuggingFace.Transformers.Model do
    def params_mapping(_spec) do
      %{
        ...
        "embedder.token_type_embedding" => "deberta.embeddings.token_type_embeddings",
        "embedder.norm" => "deberta.embeddings.LayerNorm",
        "encoder.blocks.{n}.self_attention.query" =>
          "deberta.encoder.layer.{n}.attention.self.query_proj",
        "encoder.blocks.{n}.self_attention.key" =>
          "deberta.encoder.layer.{n}.attention.self.key_proj",
        ...
        "encoder.blocks.{n}.ffn.intermediate" =>
          "deberta.encoder.layer.{n}.intermediate.dense",
