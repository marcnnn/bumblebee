defmodule Bumblebee.Audio.WavLm do
  ...
  options =
    [
      ...
      feature_extractor_norm: [
        default: :group,
        doc: """
        the normalization strategy for feature extractor layers. Can be `:group` for group
        normalization (applied to the first layer only) or `:layer` for layer normalization
  ...
  ## Architectures
    ...
    * `:base` - plain WavLM without any head on top

    * `:for_ctc` - WavLM with a CTC (Connectionist Temporal Classification)
      head for speech recognition. The head returns logits for each frame
      in the input sequence

    * `:for_sequence_classification` - WavLM with a sequence
  ...
  defp feature_extractor(input_values, spec, opts) do
    ...
    {hidden_state, _} =
      ...
      |> Enum.reduce({hidden_state, 1}, fn {{out_channels, kernel_size, stride}, idx},
        ...
        state =
          case spec.feature_extractor_norm do
            :group ->
              if idx == 0 do
            ...
            :layer ->
              Axon.layer_norm(state,
                epsilon: spec.layer_norm_epsilon,
                name: join(name, "layer_norm.#{idx}")
              )
          end
    ...
    def load(spec, data) do
      ...
      opts =
        convert!(data,
          ...
          feature_extractor_norm: {"feat_extract_norm", atom()},
  ...
  defimpl Bumblebee.HuggingFace.Transformers.Model do
    def params_mapping(_spec) do
      %{
        "feature_extractor.conv.0" => "wavlm.feature_extractor.conv_layers.0.conv",
        "feature_extractor.conv.1" => "wavlm.feature_extractor.conv_layers.1.conv",
        "feature_extractor.conv.2" => "wavlm.feature_extractor.conv_layers.2.conv",
        "feature_extractor.conv.3" => "wavlm.feature_extractor.conv_layers.3.conv",
        "feature_extractor.conv.4" => "wavlm.feature_extractor.conv_layers.4.conv",
        "feature_extractor.conv.5" => "wavlm.feature_extractor.conv_layers.5.conv",
        "feature_extractor.conv.6" => "wavlm.feature_extractor.conv_layers.6.conv",
        "feature_extractor.layer_norm" =>
          "wavlm.feature_extractor.conv_layers.0.layer_norm",
        "feature_extractor.layer_norm.0" =>
          "wavlm.feature_extractor.conv_layers.0.layer_norm",
        "feature_extractor.layer_norm.1" =>
          "wavlm.feature_extractor.conv_layers.1.layer_norm",
        "feature_extractor.layer_norm.2" =>
          "wavlm.feature_extractor.conv_layers.2.layer_norm",
        "feature_extractor.layer_norm.3" =>
          "wavlm.feature_extractor.conv_layers.3.layer_norm",
        "feature_extractor.layer_norm.4" =>
          "wavlm.feature_extractor.conv_layers.4.layer_norm",
        "feature_extractor.layer_norm.5" =>
          "wavlm.feature_extractor.conv_layers.5.layer_norm",
        "feature_extractor.layer_norm.6" =>
          "wavlm.feature_extractor.conv_layers.6.layer_norm",
        "feature_projection.norm" => "wavlm.feature_projection.layer_norm",
        "feature_projection.projection" => "wavlm.feature_projection.projection",
        "encoder.pos_conv.norm" => "wavlm.encoder.layer_norm",
        "encoder.blocks.{n}.self_attention.query" =>
          "wavlm.encoder.layers.{n}.attention.q_proj",
        "encoder.blocks.{n}.self_attention.key" =>
          "wavlm.encoder.layers.{n}.attention.k_proj",
        "encoder.blocks.{n}.self_attention.value" =>
          "wavlm.encoder.layers.{n}.attention.v_proj",
        "encoder.blocks.{n}.self_attention.output" =>
          "wavlm.encoder.layers.{n}.attention.out_proj",
        "encoder.blocks.{n}.self_attention_norm" =>
          "wavlm.encoder.layers.{n}.layer_norm",
        "encoder.blocks.{n}.ffn.intermediate" =>
          "wavlm.encoder.layers.{n}.feed_forward.intermediate_dense",
        "encoder.blocks.{n}.ffn.output" =>
          "wavlm.encoder.layers.{n}.feed_forward.output_dense",
        "encoder.blocks.{n}.output_norm" =>
          "wavlm.encoder.layers.{n}.final_layer_norm",
        "ctc_head.output" => "lm_head",
        "sequence_classification_head.dense" => "classifier.dense",
        "sequence_classification_head.output" => "classifier.output"
      }
    end
  end
end
