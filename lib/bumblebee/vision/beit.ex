defmodule Bumblebee.Vision.Beit do
  ...
  options =
    [
      vocab_size: [
        default: 8192,
        doc: "the vocabulary size of the visual token embedding for masked image modeling"
      ],
      image_size: [
        default: 224,
  ...
  defimpl Bumblebee.HuggingFace.Transformers.Model do
    def params_mapping(_spec) do
      %{
        "embedder.patch_embedding.projection" => "beit.embeddings.patch_embeddings.projection",
        "embedder.class_embedding" => %{
          "embeddings" => {
            [{"beit.embeddings", "cls_token"}],
            fn [value] -> Nx.squeeze(value, axes: [0]) end
          }
        },
        "embedder.position_embedding" => %{
          "embeddings" => {
            [{"beit.embeddings", "position_embeddings"}],
            fn [value] -> Nx.squeeze(value, axes: [0]) end
          }
        },
        "encoder.blocks.{n}.self_attention_norm" => "beit.encoder.layer.{n}.layernorm_before",
        "encoder.blocks.{n}.self_attention.key" =>
          "beit.encoder.layer.{n}.attention.attention.key",
        "encoder.blocks.{n}.self_attention.query" =>
          "beit.encoder.layer.{n}.attention.attention.query",
        "encoder.blocks.{n}.self_attention.value" =>
          "beit.encoder.layer.{n}.attention.attention.value",
        "encoder.blocks.{n}.self_attention.output" =>
          "beit.encoder.layer.{n}.attention.output.dense",
        "encoder.blocks.{n}.ffn.intermediate" => "beit.encoder.layer.{n}.intermediate.dense",
        "encoder.blocks.{n}.ffn.output" => "beit.encoder.layer.{n}.output.dense",
        "encoder.blocks.{n}.output_norm" => "beit.encoder.layer.{n}.layernorm_after",
        "norm" => "beit.layernorm",
        "pooler.output" => "beit.pooler.dense",
        "image_classification_head.output" => "classifier",
        "masked_image_modeling_head.output" => "lm_head"
      }
    end
  end
end
