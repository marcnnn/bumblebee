defmodule Bumblebee.Vision.SiglipVision do
  ...
  options =
    [
      ...
      use_attention_bias: [
        default: true,
        doc: "whether to use bias in the attention layers"
      ],
  ...
  defp core(inputs, spec) do
    ...
    # SigLIP uses mean pooling over patch tokens instead of CLS token
    hidden_state = encoder_outputs.hidden_state

    pooled_state =
      hidden_state
      |> Axon.layer_norm(epsilon: spec.layer_norm_epsilon, name: "post_norm")
      |> Axon.nx(fn x -> Nx.mean(x, axes: [1]) end, name: "mean_pool")
  ...
  defimpl Bumblebee.HuggingFace.Transformers.Config do
    def load(spec, %{"model_type" => "siglip_vision_model", "vision_config" => data}) do
      load(spec, data)
    end
